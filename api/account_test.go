		...
		},
		{
			...
			buildStubs: func(store *mock_db.MockStore) {
				store.EXPECT().
					GetAccount(gomock.Any(), gomock.Eq(account.ID)).
					Times(1).
					Return(db.Account{}, sql.ErrNoRows) //return sql.no error in real implementation
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
		...
		},
		{
			...
			buildStubs: func(store *mock_db.MockStore) {
				store.EXPECT().
					GetAccount(gomock.Any(), gomock.Eq(account.ID)).
					Times(1).
					Return(db.Account{}, sql.ErrConnDone) //return sql.no error in real implementation
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
